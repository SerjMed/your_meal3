...
orderSubmit.disabled = !cartList.length; //проверка, чтобы окно оформления заказа не выскакивало при пустой корзине
const allIdProduct = cartList.map(item => item.id) //map для перобора элементов
const data = cartList.length //проверяем кол-во
    ? await getData(`${API_URL}${PREFIX_PRODUCT}?list=${allIdProduct}`)
    : [];

//выводим кол-во товара в корзине
...
const cartController = () => {
    ...
    modalProductBtn.addEventListener('click', () => {
        addCart(
            ...
            parseInt(countAmount.textContent),//parsInt используем чтобы складывались числа при добавлении товара, а не строки
        )
    });
    ...
    //закрытие модального окна с оформлением заказа
...
export const cartInit = () => {
    cartController();
    renderCartList();//Вызываем корзину
